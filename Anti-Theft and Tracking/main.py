...
while (lock == False) and (noLock < 6): # Try get GPS 6 Times before giving up and going to Sleep
    ...
    if not all(coord): # returns false if none is in the truple
        ...
        coordHex = (hex(int(coord[0] * 100000))[2:] + hex(int(coord[1] * 100000))[2:]) # 5 dec places is plenty accurate. (4 is still good)
